...
public class MenuView {
    ...
    * */
     ...
     * */
      ...
      *  부모글에 해당하는 댓글정보 검색
      * */
     private static void inputSelectRelpyByParentNo() {
    		// 부모글의 글번호를 키보드로 입력 받는다.
    	 System.out.println("댓글을 검색할 부모글 번호는?");
      ...
      * 댓글등록
      * */
     private static void inputInsertReply() {
    		// 키보드 입력으로 댓글내용, 등록하려는 부모글번호 입력받는다.
    	 
    	
    	 System.out.println("댓글을 등록할 부모글 번호는?");
    	 int board_no = Integer.parseInt(sc.nextLine());
    	 
    	
    	 System.out.println("댓글 내용은?");
    	 String replycontent = sc.nextLine();
    	 
    	 BoardController.replyInsert(new ReplyDTO(replycontent,board_no));
